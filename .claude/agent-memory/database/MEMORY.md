...
- [Phase 2.5 Slice 4 — product_cost_history schema](project_phase25_slice4_product_cost_history.md) — migration 0007; three composite tenant FKs, new invoice_line composite unique index, created_by is NOT NULL (diverges from approvedBy's nullability on purpose)
- [Open item #2 — fill-correction ledger schema](project_open_item_2_fill_correction_ledger_schema.md) — migration 0008; count_line_write gains write_type + partial_fills_before/after; schema half only, backend must still add the ledger INSERT in editCountLineFills
