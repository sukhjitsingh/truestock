...
- [mysql-onduplicate-assignment-order](mysql_onduplicate_assignment_order.md) — MySQL/MariaDB evaluates UPDATE assignments in the table's schema column order, not the statement's or Drizzle's set{} order — a single-statement upsert can't compare a column's old vs new value on that same column
- [vendor-alias-matching-design](vendor_alias_matching_design.md) — Phase 2.5 Slice 3 vendor_alias matching: confidence formula, vendorId, single-setter rule, Docker hazard, AND the withLockRetry/1020 ER_CHECKREAD deadlock-retry fix
- [cas-replay-before-writeloop-idempotency](cas_replay_before_writeloop_idempotency.md) — Slice 4 invoice-approval idempotency: status CAS short-circuits BEFORE the per-line cost-write loop, not a UNIQUE-constraint-catch idiom; distinct from updateInvoiceStatusTx's ConflictError-on-any-mismatch shape
