...
CREATE TABLE `product` (
	...
	CONSTRAINT `product_name_size_ml_unique` UNIQUE(`name`,`size_ml`)
);
--> statement-breakpoint
CREATE TABLE `product_barcode` (
	...
	CONSTRAINT `product_barcode_barcode_unique` UNIQUE(`barcode`)
);
--> statement-breakpoint
...
ALTER TABLE `count_line` ADD CONSTRAINT `count_line_count_id_count_id_fk` FOREIGN KEY (`count_id`) REFERENCES `count`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `count_line_write` ADD CONSTRAINT `count_line_write_count_line_id_count_line_id_fk` FOREIGN KEY (`count_line_id`) REFERENCES `count_line`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
