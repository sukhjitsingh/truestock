/**
 * The Node-only half of `instrumentation.ts`, split out on purpose — see that
 * file's header for why. This module is reached ONLY via a dynamic
 * `import()` inside `register()`'s `NEXT_RUNTIME === "nodejs"` branch, never
 * imported statically. That is what keeps `node:os` and the mysql2-backed
 * domain modules below out of the edge-runtime bundle Next also builds from
 * `instrumentation.ts` for `middleware.ts` — a static top-level import here
 * would be evaluated for both runtimes regardless of any check inside a
 * function body, and the edge runtime has neither.
 *
 * Truestock's only cron: `processExtractionQueue` (extraction pipeline,
 * every 2 minutes) and `reapStuckJobs` (timeout sweep, every 5 minutes) —
 * see `lib/domain/extraction-pipeline.ts` and `lib/domain/extraction.ts` for
 * the work itself. Nothing here is authorization-relevant (mirrors
 * `middleware.ts`'s own boundary): both functions run as a system worker
 * acting across every tenant by id, the same "not a user-scoped read" design
 * `extraction_job_status_id_idx` documents.
 *
 ...
 * so a flag stashed there is the one signal `registerCron()` can check that a
